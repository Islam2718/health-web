...
import { pharmacyAreas, medicalStores } from "@/lib/pharmacy-data";
...
export function PharmacyDirectory() {
  ...
  const filtered = useMemo(() => {
    return medicalStores.filter((store) => {
      const matchesArea = area === "All Areas" || store.area === area;
      const q = query.trim().toLowerCase();
      const matchesQuery =
        !q ||
        store.name.toLowerCase().includes(q) ||
        store.categories.some((c) => c.toLowerCase().includes(q)) ||
        store.medicines.some((m) => m.name.toLowerCase().includes(q));
    ...
    <div>
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Search by store name, category, or medicine..."
      ...
      <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <AnimatePresence mode="popLayout">
          {filtered.map((store, i) => (
            <PharmacyCard key={store.id} store={store} index={i} />
          ))}
        </AnimatePresence>
      </div>
      ...
      {filtered.length === 0 && (
        <div className="mt-16 flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-4 font-semibold text-foreground">No medical stores found</p>
